...
def index():
    if request.method =='POST':
        ...
        print(len(s))
        if len(s)>81:
            return render_template({{ url_for('Template',filename='error.html') }})
        sol=solver(s)
        
        if sol==False:
            return render_template({{ url_for('Template',filename='error.html') }})
        else:
            return render_template({{ url_for('Template',filename='sol.html') }},sol=sol)
    else:
        return render_template({{ url_for('Template',filename='index.html') }})
